...
asd <- read_csv(
  ...
  select(-c("type", "averageTrueBenefit", "averageObservedBenefit"))
kableExtra::kbl(
  asd,
  format = "latex",
  escape = FALSE,
...
asd <- read_csv(
  ...
  select(-c("averageTrueBenefit", "averageObservedBenefit"))
kableExtra::kbl(
  asd,
  format = "latex",
  escape = FALSE,
...
scenarios for the sensitivity analyses can be found in Table \@ref(tab:tab3).

```{r tab3, echo=FALSE, eval=TRUE, warning=FALSE, message=FALSE}
asd <- read_csv(
  here("data/processed/analysisIdsSensitivity.csv")
) %>%
  select(-c("type", "averageTrueBenefit", "averageObservedBenefit"))
kableExtra::kbl(
  asd,
...
The results of the sensitivity analyses can be explored in Figures ... We found
no noticeable differences between methods for individualizing treatment benefit
predictions compared to the results of the simulation scenarios where baseline
covariates were assumed to be statistically independent.

```{r rmseBaseSensitivity, cache=TRUE, echo=FALSE, fig.cap="RMSE of the sensitivity analyses assuming correlated baseline covariates. All considered methods were derived using simulated samples of size 4,250 and true prediction c-statistic of 0.75", fig.show="hold", out.width = '100%'}
grid::grid.raster(tiff::readTIFF(here("figures/rmse_moderate_base_sensitivity.tiff")))
# knitr::include_graphics(here::here("figures/calibration_base.png"))
```

```{r rmseSampleSizeSensitivity, cache=TRUE, echo=FALSE, fig.cap="RMSE of the sensitivity analyses assuming correlated baseline covariates. All considered methods were derived using simulated samples of size 17,000 and true prediction c-statistic of 0.75", fig.show="hold", out.width = '100%'}
grid::grid.raster(tiff::readTIFF(here("figures/rmse_moderate_sample_size_sensitivity.tiff")))
# knitr::include_graphics(here::here("figures/calibration_base.png"))
```

```{r rmseAucSensitivity, cache=TRUE, echo=FALSE, fig.cap="RMSE of the sensitivity analyses assuming correlated baseline covariates. All considered methods were derived using simulated samples of size 4,250 and true prediction c-statistic of 0.85", fig.show="hold", out.width = '100%'}
grid::grid.raster(tiff::readTIFF(here("figures/rmse_moderate_auc_sensitivity.tiff")))
# knitr::include_graphics(here::here("figures/calibration_base.png"))
```
